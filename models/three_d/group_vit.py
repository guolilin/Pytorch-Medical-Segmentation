...
def interpolate_pos_encoding(pos_embed, D, H, W):
    num_patches = D * H * W

    N = pos_embed.shape[1]
    if num_patches == N and W == H and D == W:
        return pos_embed
    patch_pos_embed = pos_embed
    dim = pos_embed.shape[-1]
    patch_pos_embed = F.interpolate(
        patch_pos_embed.reshape(1, int(np.cbrt(N)), int(np.cbrt(N)), int(np.cbrt(N)), dim).permute(0, 4, 1, 2, 3),
        size=(D, H, W),
        mode='trilinear',
        align_corners=False)
...
def resize_attn_map(attentions, d, h, w, align_corners=False):
    f = int(np.cbrt(attentions.shape[2]))
    bs = attentions.shape[0]
    nh = attentions.shape[1]  # number of head
    groups = attentions.shape[3]  # number of group token
    # [bs, nh, d*h*w, groups] -> [bs*nh, groups, d, h, w]
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, kernel_size=7, stride=4, padding=2, in_chans=1, embed_dim=96, norm_layer=None):
        super().__init__()
        ...
        self.patches_resolution = (
            int((img_size[2] + 2 * padding[2] - kernel_size[2]) / stride[2] + 1),
    ...
    def num_patches(self):
        return self.patches_resolution[2] * self.patches_resolution[1] * self.patches_resolution[0]
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            x = self.norm(x)
        return x, dhw_shape  # [b,n,emb] (16, 16, 16)
...
class GroupViT(nn.Module):
    ...
    def get_pos_embed(self, B, D, H, W):
        if self.training:
            return self.pos_embed
        pos_embed = self.pos_embed
        pos_embed = interpolate_pos_encoding(pos_embed, D, H, W)
        return pos_embed
    ...
    def forward_features(self, x):
        ...
        x = x + self.get_pos_embed(B, *dhw_shape)
        x = self.pos_drop(x)

        group_token = None
        attn_dict_list = []
        for layer in self.layers:
            x, group_token, attn_dict = layer(x, group_token, True)
    ...
    def get_attn_maps(self, img, attn_dicts):
        attn_maps = []
